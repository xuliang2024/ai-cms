...
if (!function_exists('get_user_info')) {
    ...
    function get_user_info()
    {
        $user = session('user_auth');
        $uid = $user["uid"];
        $user_info = Db::name('admin_user')->where('id', $uid)->find();
        return $user_info;
    }
}


if (!function_exists('get_user_nickname')) {
    /**
     * 获取用户昵称
     * @param int $uid 用户id
     * @author 蔡伟明 <314013107@qq.com>
...
if (!function_exists('is_channel')) {
    ...
    function is_channel()
    {
        $user = session('user_auth');
        $uid = $user["uid"];
        $role_id = Db::name('admin_user')->where('id', $uid)->value('role');
        if($role_id == 6){
            return true;
        }else{
            return false;
