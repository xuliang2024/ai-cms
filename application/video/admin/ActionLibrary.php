...
class ActionLibrary extends Admin {
    ...
    public function index() 
    {
     ...
     public function add() 
     {
        ...
        if ($this->request->isPost()) {
            ...
            if ($data["type_id"] != '' ) {
                // code...
                $data_type_name= DB::connect('translate')->table('ts_padding_library_action')->where('id',$data["type_id"])->find();
            ...
            $r = DB::connect('translate')->table('ts_action_library_user')->insert($data);
            if ($r) {
        ...
        $type_id = array();
        $datas = DB::connect('translate')->table('ts_padding_library_action')->where('status = 1 ')->order('sort desc')->select();
        ...
        return ZBuilder::make('form')
            ...
            ->addFormItems([
                ...
                ['select', 'type_id', '选择动作类型分类', '请选择动作类型分类', $type_id],  
                ['text', 'seed', 'seed值'],
                ['text', 'comment', '备注信息'],
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $r = DB::connect('translate')->table('ts_action_library_user')->where('id',$id)->update($data);
            if ($r) {
        ...
        $info = DB::connect('translate')->table('ts_action_library_user')->where('id',$id)->find();
