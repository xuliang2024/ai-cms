...
class RoleLibrary extends Admin {
    ...
    public function index() 
    {
        ...
        cookie('ts_role_library', $map);
        
        return ZBuilder::make('table')
            ->setTableName('video/RoleLibraryModel',2) // 设置数据表名
            ->addColumns([ // 批量添加列
                    ['id', 'id'],
                    ['name', '角色名称'],
     ...
     public function add() 
     {
        ...
        if ($this->request->isPost()) {
            ...
            if ($data["style_id"] != '' ) {
                // code...
                $data_style_name= DB::connect('translate')->table('ts_role_library_style')->where('id',$data["style_id"])->find();
            ...
            if ($data["type_id"] != '' ) {
                // code...
                $data_type_name= DB::connect('translate')->table('ts_role_library_type')->where('id',$data["type_id"])->find();
            ...
            $r = DB::connect('translate')->table('ts_role_library')->insert($data);
            if ($r) {
        ...
        $style_id = array();
        $datas = DB::connect('translate')->table('ts_role_library_style')->where('status',1)->order('sort desc')->select();
        ...
        $type_id = array();
        $datas = DB::connect('translate')->table('ts_role_library_type')->where('status',1)->order('sort desc')->select();
        foreach ($datas as $data) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $r = DB::connect('translate')->table('ts_role_library')->where('id',$id)->update($data);
            if ($r) {
        ...
        $info = DB::connect('translate')->table('ts_role_library')->where('id',$id)->find();
