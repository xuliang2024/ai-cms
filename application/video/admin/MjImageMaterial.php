...
class MjImageMaterial extends Admin {
    ...
    public function index() 
    {
        ...
        $map = $this->getMap();
        // 使用动态指定的数据库连接进行查询
        $data_list = DB::connect('translate')->table('ts_mj_image_material')->where($map)
        ->order('time desc')
        ->paginate();

        cookie('ts_mj_image_material', $map);
        
        return ZBuilder::make('table')
            ->setTableName('video/MjImageMaterialModel',2) // 设置数据表名
            ->addColumns([ // 批量添加列
                ['id', 'ID'],
                ['user_id', 'user_id'],
                ['mj_jobs_id', 'mj任务表ID'],
