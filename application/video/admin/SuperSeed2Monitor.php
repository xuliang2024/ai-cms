...
class SuperSeed2Monitor extends Admin {

    protected $appName = 'st-ai/super-seed2';
    ...
    public function index()
    {
        ...
        return ZBuilder::make('table')
            ->setPageTitle('SuperSeed2 实时监控')
            ->setPageTips("自动每 5 秒刷新，展示最近 {$minutes} 分钟的 SuperSeed2 任务数据", 'info')
            ...
            ->addColumns([
                ...
                ['task_id', 'Task ID', 'callback', function($value){
                    ...
                    return "<span style='display:inline-flex;align-items:center;gap:4px;'><span class='ss2-taskid' title='{$escaped}' style='max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:inline-block;font-size:12px;font-family:monospace;'>{$escaped}</span><button type='button' class='ss2-copy-btn' data-copy='{$escaped}' style='border:none;background:#ecf5ff;color:#409eff;cursor:pointer;border-radius:3px;padding:1px 6px;font-size:12px;line-height:1.5;' title='复制'>复制</button></span>";
                }],
                ['user_id', '用户ID'],
    ...
    private function buildDashboardHtml()
    {
        return <<<'HTML'
<style>
.ss2-wrap { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif; }
.ss2-cards { display: flex; gap: 12px; margin-bottom: 16px; flex-wrap: wrap; }
.ss2-card {
...
.ss2-card:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(0,0,0,.12); }
.ss2-card .card-value { font-size: 28px; font-weight: 700; line-height: 1.2; }
.ss2-card .card-label { font-size: 13px; color: #909399; margin-top: 4px; }
.ss2-card.success { border-top-color: #67c23a; }
.ss2-card.success .card-value { color: #67c23a; }
.ss2-card.danger { border-top-color: #f56c6c; }
.ss2-card.danger .card-value { color: #f56c6c; }
.ss2-card.warning { border-top-color: #e6a23c; }
.ss2-card.warning .card-value { color: #e6a23c; }
.ss2-card.info { border-top-color: #409eff; }
.ss2-card.info .card-value { color: #409eff; }
.ss2-trend-row { margin-bottom: 16px; background: #fff; border-radius: 8px; box-shadow: 0 1px 4px rgba(0,0,0,.08); padding: 8px; }
.ss2-status-bar {
    display: flex; align-items: center; justify-content: space-between;
    padding: 10px 16px; background: #f5f7fa; border-radius: 6px; margin-bottom: 16px;
    font-size: 13px; color: #606266;
}
.ss2-status-bar .status-left { display: flex; align-items: center; gap: 12px; }
.ss2-status-bar .status-dot {
    width: 8px; height: 8px; border-radius: 50%; background: #67c23a;
    display: inline-block; animation: ss2-pulse 1.5s infinite;
}
.ss2-status-bar .status-dot.paused { background: #e6a23c; animation: none; }
@keyframes ss2-pulse { 0%,100% { opacity: 1; } 50% { opacity: .3; } }
.ss2-btn {
    padding: 5px 14px; border: 1px solid #dcdfe6; border-radius: 4px;
    background: #fff; color: #606266; cursor: pointer; font-size: 13px;
    transition: all .2s;
}
.ss2-btn:hover { border-color: #409eff; color: #409eff; }
.ss2-btn.active { background: #409eff; color: #fff; border-color: #409eff; }
</style>

<div class="ss2-wrap">
    <!-- 概览统计卡片 -->
    <div class="ss2-cards">
        <div class="ss2-card info">
            <div class="card-value" id="ss2-total">-</div>
            <div class="card-label">总任务数</div>
        </div>
        <div class="ss2-card" style="border-top-color:#9b59b6;">
            <div class="card-value" style="color:#9b59b6;" id="ss2-users">-</div>
            <div class="card-label">使用用户数</div>
        </div>
        <div class="ss2-card success">
            <div class="card-value" id="ss2-success">-</div>
            <div class="card-label">成功</div>
        </div>
        <div class="ss2-card danger">
            <div class="card-value" id="ss2-failed">-</div>
            <div class="card-label">失败</div>
        </div>
        <div class="ss2-card warning">
            <div class="card-value" id="ss2-waiting">-</div>
            <div class="card-label">等待中</div>
        </div>
        <div class="ss2-card" style="border-top-color:#409eff;">
            <div class="card-value" style="color:#409eff;" id="ss2-processing">-</div>
            <div class="card-label">处理中</div>
        </div>
        <div class="ss2-card" style="border-top-color:#67c23a;">
            <div class="card-value" style="color:#67c23a;" id="ss2-success-rate">-</div>
            <div class="card-label">成功率</div>
        </div>
        <div class="ss2-card" style="border-top-color:#f56c6c;">
            <div class="card-value" style="color:#f56c6c;" id="ss2-fail-rate">-</div>
    ...
    <div class="ss2-cards">
        <div class="ss2-card" style="border-top-color:#67c23a;">
            <div class="card-value" style="color:#67c23a;" id="ss2-avg-success-sec">-</div>
            <div class="card-label">平均成功耗时</div>
        </div>
        <div class="ss2-card" style="border-top-color:#e6a23c;">
            <div class="card-value" style="color:#e6a23c;" id="ss2-avg-wait-sec">-</div>
            <div class="card-label">平均等待耗时</div>
        </div>
        <div class="ss2-card" style="border-top-color:#fc8452;">
            <div class="card-value" style="color:#fc8452;" id="ss2-total-money">-</div>
            <div class="card-label">总扣费(积分)</div>
        </div>
        <div class="ss2-card" style="border-top-color:#f56c6c;">
            <div class="card-value" style="color:#f56c6c;" id="ss2-refund-money">-</div>
            <div class="card-label">已退款(积分)</div>
        </div>
        <div class="ss2-card" style="border-top-color:#67c23a;">
            <div class="card-value" style="color:#67c23a;" id="ss2-net-money">-</div>
            <div class="card-label">净消耗(积分)</div>
        </div>
        <div class="ss2-card" style="border-top-color:#67c23a;">
            <div class="card-value" style="color:#67c23a;" id="ss2-net-money-yuan">-</div>
            <div class="card-label">净消耗(元)</div>
        </div>
    </div>

    <!-- 趋势折线图 -->
    <div class="ss2-trend-row">
        <div id="ss2-chart-trend" style="width:100%;height:360px;"></div>
    </div>

    <!-- 自动刷新状态栏 -->
    <div class="ss2-status-bar">
        <div class="status-left">
            <span class="status-dot" id="ss2-status-dot"></span>
            <span id="ss2-status-text">正在加载数据...</span>
            <span style="color:#909399;" id="ss2-last-update"></span>
        </div>
        <div>
            <button class="ss2-btn active" id="ss2-btn-toggle" onclick="ss2ToggleRefresh()">暂停刷新</button>
            <button class="ss2-btn" id="ss2-btn-refresh" onclick="ss2ManualRefresh()">立即刷新</button>
        </div>
    </div>
</div>
HTML;
    ...
    private function buildDashboardJs($minutes)
    {
        ...
        $trendTitle = 'SuperSeed2 任务趋势 (' . ($minutesLabels[$minutes] ?? "最近 {$minutes} 分钟") . ')';

        return <<<JS
<script type="text/javascript">
(function() {
    var chartTrend  = echarts.init(document.getElementById('ss2-chart-trend'));
    ...
    function renderTable(tasks) {
        ...
        for (var i = 0; i < tasks.length; i++) {
            ...
            var taskIdCell = '<span style="display:inline-flex;align-items:center;gap:4px;">'
                ...
                + '<button type="button" class="ss2-copy-btn" data-copy="' + taskIdEsc + '" style="border:none;background:#ecf5ff;color:#409eff;cursor:pointer;border-radius:3px;padding:1px 6px;font-size:12px;line-height:1.5;" title="复制">复制</button>'
                + '</span>';
    ...
    function fetchData() {
        $.ajax({
            ...
            success: function(res) {
                ...
                $('#ss2-total').text(d.overview.total);
                $('#ss2-users').text(d.overview.uniqueUsers);
                $('#ss2-success').text(d.overview.success);
                $('#ss2-failed').text(d.overview.failed);
                $('#ss2-waiting').text(d.overview.waiting);
                $('#ss2-processing').text(d.overview.processing);
                $('#ss2-success-rate').text(d.overview.successRate + '%');
                $('#ss2-fail-rate').text(d.overview.failRate + '%');
                $('#ss2-avg-success-sec').text(fmtSec(d.overview.avgSuccessSec));
                $('#ss2-avg-wait-sec').text(fmtSec(d.overview.avgWaitingSec));
                $('#ss2-total-money').text(d.overview.totalMoney);
                $('#ss2-refund-money').text(d.overview.refundMoney);
                $('#ss2-net-money').text(d.overview.netMoney);
                $('#ss2-net-money-yuan').text('¥' + d.overview.netMoneyYuan);
                ...
                $('#ss2-last-update').text('最后更新: ' + res.time);
                $('#ss2-status-text').text('数据已更新');
            },
            error: function() {
                $('#ss2-status-text').text('数据请求失败，将自动重试');
            }
        });
    }
    ...
    function tick() {
        ...
        $('#ss2-status-text').text('下次刷新: ' + countdown + ' 秒');
    }

    window.ss2ToggleRefresh = function() {
        autoRefresh = !autoRefresh;
        var btn = document.getElementById('ss2-btn-toggle');
        var dot = document.getElementById('ss2-status-dot');
        if (autoRefresh) {
            btn.textContent = '暂停刷新';
            btn.className = 'ss2-btn active';
            dot.className = 'status-dot';
            countdown = 5;
            $('#ss2-status-text').text('已恢复自动刷新');
        } else {
            btn.textContent = '恢复刷新';
            btn.className = 'ss2-btn';
            dot.className = 'status-dot paused';
            $('#ss2-status-text').text('自动刷新已暂停');
        }
    };

    window.ss2ManualRefresh = function() {
        fetchData();
        countdown = 5;
    };

    $(document).on('click', '.ss2-copy-btn', function(e) {
        e.preventDefault();
        e.stopPropagation();
