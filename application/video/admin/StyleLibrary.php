...
class StyleLibrary extends Admin {
    ...
    public function index() 
    {
        ...
        $map[]=["creator_type","=",0];
        // 使用动态指定的数据库连接进行查询
        $data_list = DB::connect('translate')->table('ts_style_library_user')->where($map)
        ->order('sort desc')
        ->paginate();

        cookie('ts_style_library_user', $map);
        
        return ZBuilder::make('table')
            ->setTableName('video/StyleLibraryModel',2) // 设置数据表名
            ->addColumns([ // 批量添加列
                    ['id', 'id'],
                    ['name', '角色名称'],
     ...
     public function add() 
     {
        ...
        if ($this->request->isPost()) {
            ...
            if ($data["type_id"] != '' ) {
                // code...
                $data_type_name= DB::connect('translate')->table('ts_role_library_style')->where('id',$data["type_id"])->find();
            ...
            $r = DB::connect('translate')->table('ts_style_library_user')->insert($data);
            if ($r) {
        ...
        $type_id = array();
        $datas = DB::connect('translate')->table('ts_role_library_style')->where('status = 1 and if_type_role = 1')->order('sort desc')->select();
        ...
        return ZBuilder::make('form')
            ...
            ->addFormItems([
                ...
                ['select', 'type_id', '选择风格类型分类', '请选择风格类型分类', $type_id],  
                ['text', 'seed', 'seed值'],
                ['text', 'comment', '备注信息'],
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $r = DB::connect('translate')->table('ts_style_library_user')->where('id',$id)->update($data);
            if ($r) {
        ...
        $info = DB::connect('translate')->table('ts_style_library_user')->where('id',$id)->find();
