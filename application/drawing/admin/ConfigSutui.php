...
class ConfigSutui extends Admin {
    ...
    public function index() 
    {
        ...
        $data_list = DB::table('ai_config_sutui')->where($map)
        ->order($order)
        ->paginate();

        cookie('ai_config_sutui', $map);
        $contro_top_btn = ['add'];
        $contro_right_btn = ['edit','delete'];
        return ZBuilder::make('table')
            // ->setPageTitle('用户管理') // 设置页面标题
            ->setTableName('config_sutui') // 设置数据表名
            // ->setSearch(['id' => 'ID', 'username' => '用户名', 'email' => '邮箱']) // 设置搜索参数
            ->addColumns([ // 批量添加列
                    ['id', 'ID'],
                    ['channel_name','渠道名'],
                    
                    ['index_image_url', '首页封面图','img_url'],

                    ['index_image_url', '封面图','text.edit'],
                   
                    ['status', '状态','switch'],
                    ['sort', '排序','text.edit'],
                   
                    ['jpath_type', '跳转类型','status','',[1=>'外部url',2=>'内部路径',3=>'其他',0=>'不跳转']],
            ...
            ->setSearchArea([
                ...
                ['text', 'channel_name', '名字'],
                ['text', 'status', '状态'],
                ['text', 'jpath_type', '跳转类型'],
     ...
     public function add() 
     {
        ...
        if ($this->request->isPost()) {
            ...
            $r = DB::table('ai_config_sutui')->insert($data);
            if ($r) {
                // 记录行为
        ...
        return ZBuilder::make('form')
            ->addOssImage('index_image_url', '首页封面图', '', '', '', '', '', ['size' => '50,50'])
                ->addFormItems([
                // ['radio', 'type', '链接类型', '', [1 => '文字链接', 2 => '图片链接'], 1],
                ['text', 'channel_name', '渠道名'],
                ['select', 'jpath_type', '跳转类型','',[0=>'不跳转',1=>'外部url',2=>'内部页面',3=>'其他'],0],
                ['text', 'jpath', '跳转路径'],      
     ...
     public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            // $result = $this->validate($data, 'Article');
            // if(true !== $result) $this->error($result);
            $r = DB::table('ai_config_sutui')->where('id',$id)->update($data);
            if ($r) {
                // 记录行为
        ...
        $info = DB::table('ai_config_sutui')->where('id',$id)->find();
