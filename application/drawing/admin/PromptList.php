...
class PromptList extends Admin {
	
    public function index() 
    {
        $map = $this->getMap();
        $data_list = DB::table('ai_prompt_list')->where($map)
        ->order('time desc')
        ->paginate();

        cookie('ai_prompt_list', $map);
        
        return ZBuilder::make('table')
            ->setTableName('prompt_list') // 设置数据表名
            ->addColumns([ // 批量添加列
                ['id', 'ID'],
                ['prompt','正面词'],
                ['negative_prompt', '负面词'],
 ...
 public function add() 
     {
        ...
        if ($this->request->isPost()) {
            ...
            $r = DB::table('ai_prompt_list')->insert($data);
            if ($r) {
     ...
     public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $r = DB::table('ai_prompt_list')->where('id',$id)->update($data);
            if ($r) {
                // 记录行为
        ...
        $info = DB::table('ai_prompt_list')->where('id',$id)->find();
