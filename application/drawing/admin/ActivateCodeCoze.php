...
class ActivateCodeCoze extends Admin {
	
    public function index() 
    {
        $map = $this->getMap();
        $data_list = DB::table('ai_coze_activation_code_info')->where($map)
        ->order('time desc')
        ->paginate();

        cookie('ai_coze_activation_code_info', $map);
        
        return ZBuilder::make('table')
            ->setTableName('coze_activation_code_info') // 设置数据表名
            ->addColumns([ // 批量添加列
                ['id', 'ID'],
            ...
            ->addTopButton('download', [
                ...
                'href' => '/admin.php/drawing/activate_code_coze/download.html?' . $this->request->query()
    ...
    // 查询数据库
    $data_list = DB::table('ai_coze_activation_code_info')->whereIn('id', $ids_array)->select();
    ...
    public function adddaycode($day=7){
        ...
        $res = Db::table('ai_coze_activation_code_info')->insertAll($data);
        if($res){
            $this->success('添加成功');
        }else{
            $this->error('添加失败');
