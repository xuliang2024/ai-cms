            ...
            if ($this->_vars['columns']) {
                ...
                foreach ($this->_vars['columns'] as $column) {
                    ...
                    switch ($column['type']) {
                        ...
                        case 'image_video': // 单张视频
                                if ($row[$column['name']] != '') {
                                    
                                    $r = preg_match('/.*(\.png|\.jpg|\.jpeg|\.gif).*$/', $row[$column['name']]);
    
                                    if ($r) {
                                        $row[$column['name'].'__'.$column['type']] = '<div class="js-gallery"><img class="image" data-original="'.$row[$column['name']].'" src="'.$row[$column['name']].'" style="height: 400px;"></div>';    
                                    } else {
                                        // $row[$column['name'].'__'.$column['type']] = '<div class="js-gallery"><video src="'.$row[$column['name']].'" controls="controls" style="height: 100%;width:100%;object-fit:fill;" >您的浏览器不支持 video 标签。</video></div>';
                                        // <div class="m-video" data-src="jq22-sp.mp4">点我播放视频</div>
                                        $row[$column['name'].'__'.$column['type']] = '<div class="m-video"  data-src="'.$row[$column['name']].'"><img style="width: 40px;" src="https://we-spa.oss-cn-shenzhen.aliyuncs.com/total_picture/1663473318830.png"></div>';
                                    }
                                }
                                break;    
                        case 'img_url': // 外链图片
